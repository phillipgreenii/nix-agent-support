...
authority is `ADR 0001`). Where a described system instead carries governance as **intended
behavior** — an identity it acts under, a permission posture, a guardrail that must not be
config-defeatable — that **behavioral slice** belongs **in** that system's set, as ordinary
