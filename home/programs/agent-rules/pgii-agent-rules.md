...
- If the project has `.pre-commit-config.yaml` (test with `test -f .pre-commit-config.yaml && echo yes || echo no` — an exit-0 probe; do NOT probe by running the tool, and do NOT probe with bare `ls`, which exits nonzero on a missing file and is therefore itself a failed tool call — 19 such failures in the 8 days to 2026-07-30): the pre-commit hooks MUST pass on the **changed** files. The **commit's own hook run is the gate** — a `git commit` fires `prek`/`pre-commit` on the staged files (so `git add -A` first, or a generated change escapes the run). Before staging, run the project's formatter in write/fix mode over the changed files (e.g. `nix fmt -- <files>`, `gofumpt -w`, `prettier --write`) — most hook-time formatting failures are pure drift an autofix resolves, and catching it before staging avoids a failed commit + restage round-trip. (How to validate before committing without over-running the hook suite: the `nix-how-to` path-rule, `.claude/rules/nix-how-to.md`.)
- If the project has `flake.nix` (same exit-0 probe: `test -f flake.nix && echo yes || echo no`): `nix flake check` MUST pass — but as a LAND-TIME gate (once, before the branch is integrated), not a per-change/per-bead gate. Many `flake.nix` repos wire `pre-commit-hooks.nix`'s hook set into a `checks.pre-commit` derivation, so `nix flake check` re-runs the SAME checks the commit's own hook run (previous bullet) already ran — paying for the whole hook set twice per change, on top of whatever CI re-checks on push. Prefer the repo's own land-time mechanism (CI on push, or — for the two repos it's scoped to, `phillipgreenii-nix-agent-support`/`phillipg-nix-ziprecruiter` — `ff-merge-to-main`'s FF-2a) as the actual gate; for any OTHER repo, confirm it actually has an equivalent land-time gate before assuming `ff-merge-to-main` supplies one — FF-2a explicitly skips every repo but those two, so absent an external CI check, you are the gate and a mid-branch (or pre-land) manual run is still required. The match is on the canonical clone's full directory `basename`, never a shorthand — this repo is casually called `nix-agent-support` elsewhere (dropping the `phillipgreenii-` prefix); that shorthand is NOT the value FF-2a's `case` statement matches on. (How-to detail: the `nix-how-to` path-rule.)
