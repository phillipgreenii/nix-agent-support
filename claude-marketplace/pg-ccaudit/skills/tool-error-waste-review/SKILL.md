...
description: Run an agent-waste review over the Claude Code transcript corpus — both halves: tool calls that fail (error signatures, cost, blind retries) AND mistakes that were never failures (undone work, typed corrections, interruptions, rejected calls, redone files, re-issued commands). Use to audit/census agent errors or wasted time across sessions, find where agent time goes, check whether a rule/instruction fix stopped a class of error, route a fix to the right artifact (always-on rules vs. workspace rules vs. skill vs. slash command vs. subagent brief vs. hook vs. permission-approver tuning), measure correction frequency, or re-run a prior audit. Also covers retry loops, error-then-narration adjacency, hook rejections, per-tool/per-command error rates, failure concentration, and human-turn counts. Do NOT use for debugging one specific failing command now, reading a single transcript, or token/cost accounting.
---
