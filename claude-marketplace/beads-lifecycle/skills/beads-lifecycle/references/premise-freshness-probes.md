...
Full probe table for **F-3** and the runnable scripts for **F-9**'s `decided-against?` probe and
**F-10**'s `own-decision-recorded?` probe. Moved out of the main skill body to keep it within
budget (tc-ql0o Stage C, C.1/C.2); the main `SKILL.md`'s Premise Freshness section points here.
...
| **`own-decision-recorded?`** | `bd show <id> --json --include-comments \| jq -r '(if type=="object" and has("data") then .data else . end)[0] \| .acceptance_criteria, ((.comments // [])[].text)'` (full runnable form below)                                 | `acceptance_criteria` or a comment states the decision plainly (e.g. "OPERATOR DECISION", "implement BOTH", a ruling naming which option of the description's own framing was chosen) — the description's question-framing is STALE even though its raw text still poses the question | `acceptance_criteria` is absent/empty AND every comment is silent on the decision — the description's framing is accurate; a person genuinely still needs to decide |
...
| **`sibling-open?`**          | `bd show <sib-id> --json \| jq -r '(if type=="object" and has("data") then .data else . end)[0].status'`                                                                                                                        | `closed` — the bead this one waits on, duplicates, or defers to is done                                                                                                                                                                                                               | `open` / `in_progress` / `blocked`                                                                                                                                  |
...
bd list --desc-contains "$A" --status all -n 0 --json |
  jq -r '(if type=="object" and has("data") then .data else . end)[] | "== \(.id) ==\n\(.description)\n\(.notes)"' | rg -in "$R"
```
...
## F-10 `own-decision-recorded?` runnable form

Distinct from every other probe in this table: the newer record is not an external system, a
sibling bead, or a derived id — it is a LATER PART OF THIS SAME BEAD, in its `acceptance_criteria`
field (`bd update <id> --acceptance "..."`, JSON key `acceptance_criteria` — probed 2026-08-27,
see `plan-decompose-beads`'s "Field placement") or in a comment. Before treating a description that
reads as an open question ("DECISION NEEDED", "a person must choose", "operator: pick (a) or (b)")
as still live — and specifically BEFORE parking or re-parking the bead on that framing — the agent
MUST run this probe and MUST record its output:

```bash
bd show <id> --json --include-comments | jq -r '(if type=="object" and has("data") then .data else . end)[0] |
  "ACCEPTANCE: \(.acceptance_criteria // "none")\n---\n" +
  ((.comments // []) | map("[\(.author) \(.created_at)] \(.text)") | join("\n---\n"))'
```

(`bd comments <id>` — not `bd comment list` — is the equivalent standalone command for the comment
half, if `--include-comments` is unavailable on the installed `bd`.) Read the output for decision
language: "OPERATOR DECISION", "decided", "implement BOTH" / "implement (a)", or any ruling that
names which of the description's own options was chosen. A hit is DECISIVE (**F-10**): the
description's own question-framing is STALE, the bead reads as ORDINARY WORK, and it MUST NOT be
parked or re-parked on that already-answered question — no matter how the raw description text is
still worded. Silence in BOTH `acceptance_criteria` and every comment means the description's
framing is accurate: proceed to classify the bead as a genuine human question as normal.

Live worked case (`tc-uqw6s`, 2026-09-15): `tc-oyp6` and `tc-ts8f` already stated the decision in
`acceptance_criteria`; `tc-ldoz` and `tc-a8f8s` had it only in a comment. All four were re-parked
as `human` anyway because the triage step ran no probe over either field — this probe closes
exactly that gap.
