...
This skill was moved out of the always-on `~/.claude/CLAUDE.md` (tc-ql0o Stage C, operator
Decision 1/2/3, 2026-08-25/26): these rule packs all key on an observable trigger — a `bd`
verb, a park/release/accept action, or a label mutation — so a MUST-invoke tripwire in the
always-on core (naming this skill) can gate them instead of the full text riding in every
session's context unconditionally. A bare prohibition or conversation-time ruling with NO
such trigger (S-1/S-2, T-1..T-3) stays in core, unmoved — see that file's stub for the
rule-ID family map.

Sections below are ordered with the invariants that have the narrowest / most easily missed
violation windows FIRST (per this skill's own design record): claim hygiene's release step,
the worktree-review label's atomic-release ordering, then blocker modeling, handoff
...
> contract lives HERE, in this skill (moved from the always-on rules file by operator
> Decision 3, tc-ql0o Stage C, 2026-08-25/26 — superseding this blockquote's earlier claim
> that it "lives HERE, in the always-on rules, and not in a script": the MUST-invoke tripwire
> in core now serves the same binding purpose without the full text riding in every session).
...
- **W-7** If the W-3 read-back finds NO `Promoted` token, the pre-promotion priority is
  ...
  that same exchange and use their answer. Only an operator who declines leaves it at P<n>. A
  narrow provably-lossless-close exception exists — see
  `references/worktree-review-exceptions.md`.
...
- **D-5** ORDERING IS LOAD-BEARING: every dependency edge MUST be added BEFORE the claim is
  ...
  (the same reasoning as the worktree-review W-6 close ordering above).
...
- **D-8** THE REVERSE CONVERSION IS MANDATORY TOO, AND IT NEEDS NO PERSON. An issue already labeled
  ...
  (the `sibling-open?` probe, see Premise Freshness below) is not a blocker at all and MUST NOT be
  given an edge — that issue's label reason simply died. A dependency MUST NOT be substituted by a
  DEFER window either: a defer is a TIMER that expires whether or not the blocker cleared, while an
  edge IS the state.
