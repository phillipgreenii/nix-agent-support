...
- **R3 (S1) — five surfaces, not three.** `IsAnyBusy` (`server.go:135-144`) and the `agents-busy-check` exit-code contract sum `WorkingN`; and `cmuxstatus/reporter.go:15-20` has states `{Unknown,Dormant,Idle,Working,Paused}` with no `Blocked`. Decision: `IsAnyBusy`/busy = `status==working` only (preserves current semantics; a `blocked` session is not "actively progressing"). `cmuxstatus` maps `blocked`→`Paused` (extending its existing `Paused` notion) rather than losing it to `Idle`. **Caller consequence — documented, not a regression** (the gate analogue of R5's keepAwake consequence): because `busy` excludes `blocked`, every gate keyed off it reports success mid-window with blocked work still pending — `agents-busy-check` prints `all idle` and exits 1, and `wait-until-agents-finished` exits 0 ("idle reached") after N ticks of `WorkingN == 0`, as does the packaged `wait-for-agents-to-finish` wrapper that delegates to it. "Idle reached" therefore does NOT mean "work finished": a caller that must not proceed until pending work is genuinely done MUST also consult the `blocked` count (e.g. `pa-monitor status`) rather than the exit code alone. Recorded for callers in `packages/pa-monitor/README.md` § "Busy/idle gates", `packages/wait-for-agents/README.md` § "Exit Codes", and the two subcommands' doc comments. Whether a `usage_limit`-blocked session SHOULD instead hold those gates open is left as an open question for pa-monitor's behavior-docs set; until a superseding decision, this R3 semantics stands.
