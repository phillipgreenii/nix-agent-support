...
- a private monorepo — claude-code darwin module, pg2-agent, beads services,
  serena, zw-reset-agents, zw-agent-activity

Duplication (e.g., two `ccusage` modules, two local plugin marketplaces) and unclear
ownership made the tooling hard to navigate, extend, and test. Consumer-specific bits were
...
existing three repos. All generic agent/AI modules move here. Consumer-specific bits stay in
a private monorepo as thin option-level extensions.

`phillipgreenii-nix-personal` and `phillipgreenii-nix-support-apps` lose their agent
modules and gain no new dependency. A private monorepo gains
...
and have no agent-specific responsibilities. Consumer extension points are explicit and
auditable. Enables future use of agent tooling outside the consumer machine context.

### Negative

Additional flake to maintain, update-lock, and keep in sync with nixpkgs versions.
The consuming flake must add a new flake input.

### Neutral

Follows the existing pattern: one flake per concern, composed at the consumer machine level.
