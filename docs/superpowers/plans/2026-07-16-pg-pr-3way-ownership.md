...
- **[S1 — Task 11] Existing builder tests break.** Switching `Build`'s partition to `p.Ownership.ActsAsMine()` means every `PRInput` literal needs `Ownership` set. The existing tests in `internal/snapshot/builder_test.go` build self-authored `PRInput`s with NO `Ownership` (→ `""` → dropped from both panels). Add `Ownership: ownership.Mine` to the self-authored `PRInput` in EACH of: `TestBuildSplitsMineFromReview`, `TestBuild_MinePRStaysMineEvenDraft`, `TestBuildDerivesApprovalAndWaiting`, `TestBuildEmptyArraysNotNil`, `TestBuildAgentApprovedViaInlineCommentIgnored`, `TestBuildExcludesAdvisoryCIChecks`, `TestBuildNilRegistry`, `TestBuildMineRowMergeReminder`. (Task 11 Step 4's "still green" note is WRONG without this — those tests call `Build()` directly, not `buildPRInput`.)
- **[import shadowing — Tasks 4/5/6/7] `ownership` name collision.** Local variables literally named `ownership` exist at `sync.go:437`, `sync.go:1259` (`applyFetchedPR`), `refresh.go:63`. Importing package `internal/ownership` shadows them. Rename EVERY such local to `own`/`ownershipStr` (not only the ones quoted in snippets) or `ownership.Classify` won't resolve.
- **[S4 — Task 7] `emitAttention` test call sites.** `go build` does NOT compile test files, so the per-task `go build` gates miss this; only `go test ./internal/sync/` does. `internal/sync/attention_emit_test.go` calls `emitAttention` at lines ~77, 112, 133, 188 — update all four for the new `own` argument.
- **[S5 — Task 6] Ingest test harness (no `newIngestTestEngine`).** Build the engine inline as the existing ingest tests do (`internal/sync/ingest_test.go:77-89`): `e := New(Deps{Cfg: &config.Config{SelfLogin: "me", Repos: …}, VCS: …, Beads: &noopBeads{}, StateDir: t.TempDir(), Store: db})`, then call `e.ingestFeedbackToStore(ctx, "o/r", pr, enriched)`.
- **[S3 — Tasks 9,10] Enumerate ALL `beadsbridge.BeadClient` fakes.** The compile-time assertion forces `*beads.Client` (fine — new methods are real). Every FAKE must gain no-op `EnsureDraftReviewMineLabel` + `SetMergeRequestCoOwned`: in `internal/beadsbridge/bridge_test.go` → `noopBeadClient` (~51), `capturingClient` (~131), `upsertClient` (~255), `scenarioClosedClient` (~305), `draftReviewClient` (~411); in `internal/sync/` → `concurrentBeadClient` (`concurrency_test.go:119`), `outboxFakeBeads` (`refresh_test.go:344`), `fullChainBeadClient` (`integration_test.go:86`); in `cmd/pg-pr/` → `stubBeads` (`sync_test.go:46`), `fakeBeadsClient` (`pr_write_test.go:143`). **Add `cmd/pg-pr` to the Files list** of Tasks 9/10. Do NOT touch `noopBeads` (`sync_test.go:1012`) — it is the slim `sync.BeadClient`, not `beadsbridge.BeadClient`.
- **[Task 12 — resolved by critique]** `detector.go:181` (`openBeadsForGroup`) only routes refresh-enqueue (mine vs team queue); `refreshPR` re-derives ownership regardless, so a co-owned PR is not misrouted. Task 12 → STOP with a one-line comment; no code change.
- **[verified OK]** Task 4 loop reorder is safe (per-repo enrich runs in a synchronous IIFE, so `enrichByRepo` is fully populated; nothing between emit and the pluck depends on ordering; `reconcileTruncatedCI` touches only `CIRuns`). Task 7 enrich-early is safe (closed/merged return first; `enrichOnePR` is fine on drafts). Migration v8 column list is exact. `buildPRInput` is the single populated-`PRInput` site. `maybePromoteDraft` is the ONLY inappropriate co-owned upstream write (Merge is manual CLI; PostReview routes co-owned's mine-label to the local minesink, no upstream write).
