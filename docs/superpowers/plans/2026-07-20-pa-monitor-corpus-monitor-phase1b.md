...
- **Import direction (no cycles):** `corpus` MAY import `session`, `transcript`, `usage`, the new `limits`, and `claude-transcript` (`ct`); none of those may import `corpus`. `transcript` already imports `usage` (snapshot.go:14) — that stays. The new `limits` package is a **stdlib-only** leaf (encoding/json, os, time — the `IsStatusSiblingFile` discovery stays in the caller, so `limits` needs no `session` import). `daemon` MAY import `limits` and `corpus` (via `poller`); `limits`/`corpus` MUST NOT import `daemon`.
...
- [x] **Step 3 — DONE** `limits.go` created (stdlib-only leaf; `Current` guards empty→nil); `daemon` re-pointed via `type Limits = limits.Limits` alias (ports.go) + `SiblingLimitsSource.Current` delegates to `limits.ReadStatusRecords`+`limits.Current`; moved funcs deleted from `sibling_limits.go`. **`sibling_limits_test.go` kept FULLY INTACT** (all disk-discovery + ADR-0029 disk tests still pass through `SiblingLimitsSource` — full disk coverage retained, a positive deviation from the planned "trim"); in-memory `limits_test.go` added as the permanent fold suite. `applyLimits`/`lifecycle.go` needed no change (alias transparent).
