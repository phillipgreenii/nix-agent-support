...
- The `cp -R …/. $out/` (trailing `/.`) copies the _contents_ so `$out/go.mod` exists (not `$out/pg-pr/go.mod`).
...
> the gascity pack wiring), then Task 7 is reduced to: set the `realBinary` _option default_ to a
