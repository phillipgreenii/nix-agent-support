...
| `amend-design(docket, design-or-diff, revision+1)`       | RECONCILE entry point; accepts a diff against the current design (binding resolves the merge) or, with no prior design revision, full replacement text; MUST handle the medium's large-design storage (§11) |
...
Input: a docket and either a diff against its current design or, when the docket has no prior
design revision yet, full replacement text; the binding's `amend-design` RESOLVES this into
the full amended design text (patch-applying a diff, or using full text as-is) before
anything is written to the docket (§11). Order is load-bearing: run the §8.1 pre-check on
the RESOLVED AMENDED design FIRST; only after it passes, `amend-design`'s Commit step (bump
`pd_rev`, supersede per S-2 — struck/rewritten text, ruling recorded), setting
`pd_phase=reconciling:<new-rev>` at
...
| `amend-design`                       | Two phases: RESOLVE the merged text from either a diff against the current design (patch-applied against a scratch file: `patch`/`git apply --unidiff-zero`, a rejected hunk HALTs with a gap report) or, with no prior design revision, full replacement text accepted as-is; then, only after the §8.1 pre-check passes the resolved text, COMMIT it — rewrite the DESIGN field (S-2: superseded text struck/rewritten, ruling recorded verbatim); for chunked designs, rewrite the header/index and append new-revision chunks, striking superseded chunks in the index (comments cannot be edited); bump `pd_rev` via `--set-metadata` |
...
- **Reconcile:** "the design for docket `<id>` changed — reconcile" → mode `reconcile` with a
  diff against the current design, or full text when the docket has no design revision yet.
...
- ~~Diff-based `amend-design` (v1 takes the full amended text; a patch form would cut the
  reconcile resupply cost for large designs).~~ Implemented (bead `pg2-4wktv`, 2026-08-28):
  `amend-design` now accepts a unified diff against the current design, resolving the merged
  text via `patch`/`git apply` before committing it, and falls back to accepting full
  replacement text unconditionally when the docket has no prior design revision to diff
  against — see §11's `amend-design` mapping and §8.11's mode `reconcile`.
