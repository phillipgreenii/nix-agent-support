...
// mustNotReachAChild lists every GIT_* variable that can redirect a child
// at a repository, an index, an object store, a discovery boundary, or an
// injected config value — including the leaked-by-a-hook set the sibling
// backend's identical package guards against (worktree add/remove are the
// two mutating verbs this backend issues that a leak would misdirect).
...
// exists for: if any of these reaches the git child, `-C dir` is a lie and
// a mutating verb (worktree add/remove) acts on the wrong repository.
func TestHermeticDropsEveryRepoRedirectingVar(t *testing.T) {
	got := envKeys(Hermetic(mustNotReachAChild))
	if len(got) != 0 {
...
// contract: production git needs the ambient non-GIT_ environment
// (transport, home, locale, temp dir), so filtering must not degrade into
// a full allowlist.
...
// membership rule mechanically: nothing in it may end in a
// location-shaped suffix.
...
// TestCommandEnvExcludesLeakedGitDir asserts on the constructed
// exec.Cmd.Env, so it needs no subprocess and no real repository.
func TestCommandEnvExcludesLeakedGitDir(t *testing.T) {
	for _, kv := range mustNotReachAChild {
		k, v, _ := strings.Cut(kv, "=")
		t.Setenv(k, v)
	}

	cmd := Command(context.Background(), "/some/target/dir", "worktree", "remove", "/some/target/dir")
...
func TestCommandBuildsDirScopedArgs(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: []string{"git", "-C", "/target", "rev-parse", "--show-toplevel"},
		},
		{
			name: "empty dir omits -C",
			dir:  "",
			args: []string{"rev-parse", "--show-toplevel"},
			want: []string{"git", "rev-parse", "--show-toplevel"},
		},
		{
			name: "no args",
			dir:  "/target",
