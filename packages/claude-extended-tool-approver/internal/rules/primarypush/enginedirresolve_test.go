...
// pg2-f6cgn / commit 98f8c95d in packages/pb (the primarycommit-side twin of this
// fixture, in internal/engine, hit this live). That denylist already missed
...
// launched `go test` silently redirects every "isolated" `-C canonical` call here onto
// whatever repository that variable names instead — confirmed by reproduction:
...
// since the caller relies on t.Setenv-ing it to "/dev/null" explicitly).
//
...
func hermeticEnviron(t *testing.T) []string {
	...
	// files. GIT_CONFIG_GLOBAL/_SYSTEM: forwarded so the caller's t.Setenv override
	// (this fixture points them at /dev/null) actually reaches the subprocess. None of
	// these four names a git repository location.
