...
func (c *capturingEvaluator) EvaluateStructure(source string, leaves []cmdparse.ParsedCommand, stack []hookio.StackFrame, origin *hookio.HookInput, _, _ map[string]string) hookio.RuleResult {
	c.lastOrigin = origin
	c.lastSource = source
	c.lastLeaves = leaves
	return c.result
