...
| Enforcement guard 3 (parse-count, I7)                 | **PRECONDITION LARGELY SATISFIED — step 5 (`pg2-0gsy5`).** ADR 0039's Enforcement states guard 3 MUST land AFTER the per-rule `gitdir` migration, because `gitdir` unconditionally re-parsed the root expression and recursed to depth 8 for EVERY consuming leaf. `gitdir.go`'s `scopeLeaves` now walks the already-lowered `ParsedCommand.Substitutions` / `Substitution.Leaves` tree instead, with no depth bound needed — for the ARGS/REDIRECTIONS half of a leaf. ONE narrow exception remains, found by corpus replay (not assumed): an assignment's own VALUE (`ORIG=$(cat "$GD/x")`) is never lowered into `Substitutions` anywhere in cmdparse for a plain `NAME=VALUE` leaf (only the export/declare-family path does, via `lowerDecl`), so `gitdir.envValueSubstitutionLeaves` still calls `cmdparse.Parse` on such a value's substitution body when one exists — scoped to that one value string, never the whole expression, and never unconditionally (most assignments carry no substitution at all). Guard 3 ITSELF is not implemented by this bead — that is `pg2-x9452`, which should decide whether this residual, value-scoped parse is within its budget or whether cmdparse should grow an assignment-value `Substitutions` field to close it structurally. |
...
- Step 5 may now assert `matchParen`'s removal as DONE rather than owing it.
  ...
  **UPDATE (`pg2-0gsy5`, step 5's gitdir half):** gitdir's two scanners named above are now
  DONE. `scopeLeaves` is deleted and replaced by a walk over already-lowered structure
  (`ParsedCommand.Substitutions` / `Substitution.Leaves`, threaded via `pipeScope.rootLeaves()`)
  for a leaf's own args/redirections. ONE exception was found EMPIRICALLY, by corpus replay
  against a base tree still carrying the outgoing `scopeLeaves` (4,784 `.git`-bearing corpus
  rows; two transitioned less-restrictive before the exception was added, zero after): an
  assignment's own VALUE is never lowered into `Substitutions` for a plain `NAME=VALUE` leaf
  anywhere in cmdparse, so `gitdir.envValueSubstitutionLeaves` still parses such a value's
  substitution body when one is present — scoped to that value, not the whole expression, and
  not unconditional. `containsVarRef` is deleted; its boundary-matching logic survives as the
  smaller, private `hasNameRef`, but it is now reached only through `referencesVar`, which gates
  on `ParsedCommand.ArgIsLiveExpansion` before scanning an Args operand at all — the "no quote
  comparison" gap the old scanner had, closed by consulting the parser's own live-expansion fact
  instead of re-deriving it from unquoted bytes (pg2-rz9ds). Remaining residue for `pg2-x9452` is
  now just docker's `splitOnShellOperators` and envvars' value scan, plus guards 2 and 3 (guard
  3's own budget must decide whether gitdir's value-scoped residual parse above is acceptable or
  whether cmdparse should grow an assignment-value `Substitutions` field instead) and the I13
  structural delegate entry point.
