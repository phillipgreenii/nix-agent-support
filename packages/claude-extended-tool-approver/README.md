...
14. **git** -- git subcommands. `git tag` → Reject. On `git push`, Reject every spelling of a force-push (`-f`, `--force`, a clustered `-f…`, a `+`-prefixed refspec), of a remote-ref delete (`--delete`, `-d`, a `:ref` refspec), and `--mirror`; also Reject `--force-with-lease` when the refspec is CROSS-BRANCH. Same-branch `--force-with-lease` stays Approve (the post-rebase idiom). `git branch -D` and `git reset --hard` / `git clean` Ask
