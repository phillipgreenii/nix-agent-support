...
15. **gh-stack** -- hardcoded (not consumer-configured) per-subcommand allowlist for the `gh stack <verb> ...` GitHub CLI extension (bead `pg2-s4lzw`, follow-up to `pg2-z3u4f`'s conformance enumeration finding the plugin's ENTIRE prescribed command surface -- `view`/`up`/`down`/`top`/`bottom`/`trunk`/`checkout`/`init`/`add`/`push`/`submit`/`link`/`sync`/`rebase`/`merge`/`unstack` -- abstained through the whole rule chain, since `gh` itself only recognizes its own built-in resources, not this third-party extension). `view`/`up`/`down`/`top`/`bottom`/`trunk`/`init`/`unstack --local`/`add <branch>` bare (no `-A`/`-u`/`-m`) → Approve (read-only, navigation-only, or local-only, no PR/GitHub-API side effect); `checkout <branch-name>` → Approve (resolves against LOCALLY TRACKED stacks only, per the vendored SKILL.md), but `checkout <stack-number|pr-number|pr-url>` → Ask (a real network fetch); `unstack` / `unstack <number>` without `--local` → Ask (a real GitHub API write, though PRs/branches are never deleted); `push`/`submit --auto[--open]`/`link`/`sync` → Ask (each a real remote mutation, or -- for `link`/`submit` -- an open question the package doc records for a future revisit); `rebase` (every flag/form) → **Approve**, mirroring plain `git rebase`'s verdict in `internal/rules/git` rather than its own Ask bucket, since gh-stack's rebase has no interactive/`-i` mode at all; `merge` (every flag/form) → **Reject**, mirroring `gh pr merge`'s immediate-merge Reject exactly, with no deferred/queued spelling to carve out. `gh stack add -A`/`-u`/`-m <msg> <branch>` (bundling an uninspected commit) is deliberately left UNCLAIMED rather than claimed at any verdict -- CETA cannot see what is being committed, mirroring why a plain `git commit` is not blanket-approved -- and so is `gh stack modify` (the SKILL.md's own exit-code table states this skill never produces it)
...
21. **pn-workspace** -- hardcoded (not consumer-configured) allowlist for the routine, non-destructive `pn workspace` subcommand family, per operator ruling (Phillip, 2026-09-17, `pg2-4zyqf`/`pg2-vlpe1`): `pn workspace build|status|push|doctor|update` and `pn workspace workforest add|list` → Approve. `pn workspace workforest remove` and any direct write to `pn-workspace.toml` are explicitly excluded and keep their prior (non-relaxed) scrutiny -- the latter is not even a `pn` invocation, so this rule never reaches it at all. Any other/unlisted subcommand Abstains
22. **kill-probe** -- hardcoded allowlist for the `kill -0`/`kill -s 0`/`--signal 0`/`--signal=0` null-signal liveness probe (ADR 0074, `pg2-z3u4f` item 1): Approve, because signal 0 sends nothing -- it is an existence/permission check only, so the pid operand(s) are irrelevant to safety regardless of spelling. Any OTHER `kill` invocation (the default SIGTERM, a real numeric/named signal, `-l`/`-L` listing, or an unrecognized flag) is left unmatched -- `kill` is deliberately absent from `dangerous-commands`' denylist (see that rule's own scope note) and unclaimed by every other rule, so today it Abstains, unchanged by this rule
23. **pg-ccaudit** -- hardcoded (not consumer-configured) allowlist for the first-party pg-ccaudit CLI's command family (bead `pg2-xu4aq`, follow-up to `pg2-amzvw`'s plugin-conformance-check allowlisting it), classified per `packages/pg-ccaudit/cmd/pg-ccaudit/main.go`'s own usage text: `status`/`query`/`queries`/`schema`/`candidates`/`classify`/`report`/`evaluate`/`gold`/`cost`/`version`/`help` (its "read-only" band, and metadata commands) and a bare invocation → Approve; `ingest` (a real, potentially long-running write to the persistent index, which the tool-error-waste-review skill requires operator authorization before running) → Ask. Any other/unlisted subcommand Abstains
24. **safecmds** -- safe commands with path checks (runs AFTER curl/ssh/vault so a configured command-aware leaf is decided by its dedicated rule). A path argument that is DYNAMICALLY EXPANDED (`$VAR`, `${VAR}`, `$(…)`, backtick) is not statically determinable, so the command is NOT approved -- for READS (`cat`/`head`/`tail`/`less`/`more`/`wc`/`diff`/`sort`/`uniq`/`awk`/`jq`/`tq`/`xxd`/`strings`, plus `sed`/`yq`/`gofmt`/`grep`/`rg` reads, `jar tf|xf`, `bash -n`) exactly as for writes (`rm`/`cp`/`mv`/`mkdir`/`touch`/`chmod`/`tee`). Reads were previously exempt, and one variable hop therefore erased the credential deny-list entirely: `cat ~/.ssh/id_rsa` denied while `F=~/.ssh/id_rsa; cat $F` auto-approved. A credential read is an exfiltration primitive, so the read path warrants the refusal at least as much as the write path. Three exceptions, each deliberate: the **browsing** commands (`ls`/`find`/`du`/`stat`/`file`/`lsof`) expose names, sizes and timestamps but never file CONTENT, so a dynamic path there is not gated; the PROGRAM operand of `awk`/`sed`/`jq` is code whose literal `$` (an awk field reference, a sed end-of-line anchor, a jq `--arg` variable) is not a shell expansion, so it is judged only against the bare-expansion shapes (`awk $F` is still refused, `awk '{print $1}' f` is not); and the refusal is a NON-APPROVAL (the call returns to Claude Code's prompt), not a deny -- restoring a deny would require resolving the variable by intra-command dataflow. `shift` (`pg2-xu4aq`) is unconditionally safe (`alwaysSafe`): it only mutates the invoking shell's own positional-parameter list, and its sole optional operand is a shift count, never a file/path -- same class as `break`/`continue`/`return`/`exit`
25. **kubectl** -- Kubernetes operations (`kubectl` block extensions)
26. **buildtools** -- gradle, pre-commit, bats, etc. (`buildtools` block extensions). The base (non-consumer-configured) tool set also includes two first-party plugin-prescribed entries added by `pg2-xu4aq`: `pg-go-mutate` (the mutation-testing driver `claude-marketplace/pg-go-mutate`'s go-test-gaps skill prescribes -- no more permissive than the already-approved bare `go`, since it only ever runs the target package's own existing test suite) and `create-child-bead.sh` (the pg-pr-break-down-work skill's bundled bd-create wrapper -- no more permissive than the already-approved bare `bd`, since its entire effect surface is one `bd create` call built from its own argv)
27. **sqlite3** -- sqlite3 read/write/DDL classification
