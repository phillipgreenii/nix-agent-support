...
// Integration tests for the ceta hook/CLI binary (bead tc-fqu7 fallout).
//
// EVERY test in this file EXECS THE COMPILED BINARY — via runHook, runCLI, or a
// direct exec.Command(cliBinary) — and most of them drive a real SQLite ask log
// through it. They are correctness gates, not unit tests, and they are split out
// behind the `integration` build tag so the DEFAULT `go test ./...` (which is
// what mkGoApp's check phase and mkGoTest both run) never sees them.
//
// WHY THE TAG EXISTS. The package build for a nixosConfiguration runs mkGoApp
// with subPackages = ["cmd/claude-extended-tool-approver"], which scopes
// gomod2nix's check hook to THIS package alone — so these tests, and not the
// ~1,020 internal/* unit tests, are what a monorepod deploy actually runs. Each
// exec'd binary opens an ask log whose creation costs durable flushes, and fsync
// latency is a HOST property spanning orders of magnitude (~0.8us on tmpfs,
// ~50ms on a healthy ext4, 1.1-3.6s on a QEMU guest whose host array has lost
// its write cache). Measured on 2026-08-16: 50 of these passed in 559.33s and
// the 51st was still running when `go test`'s 10m alarm fired, failing the whole
// deploy — with ZERO test failures. The suite's wall clock was a multiple of an
// unbounded host property, so no -timeout can be chosen that a slower disk
// cannot blow through. Tagging removes them from the build rather than budgeting
// for them.
//
// RUN THEM DIRECTLY:
//
...
//
// CI runs them too, as the `claude-extended-tool-approver-integration-tests`
// flake check (mkGoTest with testFlags = ["-tags" "integration"]). That check is
// reached by `nix flake check`, NEVER by a package/nixosConfiguration build, so
// a slow disk can no longer block a deploy.
//
// This mirrors the repo's existing tag idiom — pa-monitor's `//go:build hostile`
// for tests that spawn the built daemon, pb's `contract`/`smoke` — all sanctioned
// by ADR 0021.
...
// cliBinary is the compiled binary under test. It is built LAZILY, by the first
// test that needs it, rather than in TestMain: TestMain is untagged and so also
// runs for the unit-only pass, and compiling a binary no unit test execs would
// put a `go build` — and its I/O — back on the build path this tag exists to
// clear. buildOnce makes the compile happen exactly once per `go test` process,
// no matter how many tests ask for it.
var (
	cliBinary string
	buildOnce sync.Once
	buildErr  error
...
// testBinary returns the path to the compiled binary, building it on first call.
// It lives under testScratchDir (see main_test.go), so TestMain's single
// RemoveAll cleans it up along with everything else this package writes.
func testBinary(t *testing.T) string {
	...
	buildOnce.Do(func() {
		...
		binDir := filepath.Join(testScratchDir, "bin")
		if err := os.MkdirAll(binDir, 0o755); err != nil {
			buildErr = err
			return
		}
		path := filepath.Join(binDir, "claude-extended-tool-approver")
		build := exec.Command("go", "build", "-o", path, "./cmd/claude-extended-tool-approver")
		build.Dir = root
	...
	if buildErr != nil {
		t.Fatalf("could not build the binary under test: %v", buildErr)
	}
	return cliBinary
}
