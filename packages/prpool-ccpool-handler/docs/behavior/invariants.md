...
Realization gaps — none of the code these rules govern exists yet). A rule pr-pool's own core
already states (event shape, at-least-once delivery, the offer/decline machinery) is not restated
here — see `packages/pr-pool/docs/behavior/invariants.md`. These are the rules specific to this
module as an **implementer** of `INTF-HANDLER`/`INTF-SOURCE`.

- **`INV-CCH-1`** — this module's Go module MUST import `packages/pr-pool` (its wire schemas and
  `conformance` package) and `packages/pr-pool` MUST NOT import this module back. The dependency
...
- **`INV-CCH-3`** — a handler session's post-accept outcome (`retryable`, `resource-limit`,
  ...
  `INTF-HANDLER` already grants (`packages/pr-pool/docs/behavior/interfaces.md`'s `INTF-HANDLER`
...
- **`INV-CCH-4`** — this module's own coverage-thresholds file
  ...
  into, `packages/pr-pool/tests/coverage-thresholds.txt`. The module boundary is also the
...
- **`INV-CCH-5`** — this module MUST NOT name a concrete backing tool (`ccpool`, `bd`, `pg-pr`, or
  any operator-configured command) anywhere in `packages/pr-pool`'s own contract surface (its
  `--help` text, config schema, or wire messages). Naming a backing tool is this module's own
  business, per the Floor stated in both this set's and pr-pool's own `## Scope`.
