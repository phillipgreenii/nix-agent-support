...
`prpool-ccpool-handler` is `packages/pg-router`'s **downstream deployment set** for the
...
| -------------- | ------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| `INTF-HANDLER` | this module realizes `INTF-HANDLER` for the ccpool-backed and command-backed handler kinds | this module is an empty, buildable shell — the ccpool executor and the command executor still live in `packages/pg-router`                      | `pg2-oju6w.2`, `pg2-oju6w.3` |
| `INTF-SOURCE`  | this module realizes `INTF-SOURCE` for the beads-backed built-in query set                 | not yet moved — the beads client and built-in query set still live in `packages/pg-router`'s `internal/beads`/`internal/roles`/`internal/query` | `pg2-oju6w.2`                |
