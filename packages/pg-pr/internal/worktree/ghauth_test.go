...
// ---------------------------------------------------------------------------
// site: internal/worktree — CLIGHClient.PRExists
// ---------------------------------------------------------------------------

// TestPRExists_NoToken_DoesNotExecGH proves the worktree PR precheck cannot
// reach an unauthenticated gh.
func TestPRExists_NoToken_DoesNotExecGH(t *testing.T) {
	marker := ghStubOnPath(t)
	spy := &spyCommander{inner: github.NewCLIWithTokenSource(ghFailTokenSource{})}
	c := &CLIGHClient{gh: spy}

	info, err := c.PRExists(context.Background(), "owner", "repo", 7)
	if err == nil {
		t.Fatal("PRExists = nil error with no resolvable token")
	}
	if info != nil {
		t.Errorf("PRExists returned %+v, want nil", info)
	}
...
// TestPRExists_WithToken_InjectsSingleGHToken asserts the child env of the gh
// this site invokes once a token IS resolved.
func TestPRExists_WithToken_InjectsSingleGHToken(t *testing.T) {
	...
	if _, err := c.PRExists(context.Background(), "owner", "repo", 7); err == nil {
