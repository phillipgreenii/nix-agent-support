...
func (e *Engine) refreshPR(ctx context.Context, repo string, number int) (*snapshot.PRInput, error) {
	...
	// Teammate-attention BEAD projection (pg2-4c5i.13) was removed by
	// pg2-ynhr.5: it emitted pr.attention purely to drive beadsbridge's
	// attention-bead production, which shipped off with the rest of the legacy
	// review workflow. The dashboard's OWN attention verdict
	// (snapshot.NeedsAttention, read fresh from store facts by buildPRInput
	// below) is UNRELATED and unaffected (FORK2a).

	// Single per-refresh flush: drains pr.opened/updated and any
	// feedback.created in one outbox pass. MUST precede buildPRInput, whose dep
	// path locates the bead via FindByRepoAndNumber — the bead does not exist
	// until this flush projects it.
	flushOutbox(ctx, e.deps.Store, e.deps.Dispatch)
