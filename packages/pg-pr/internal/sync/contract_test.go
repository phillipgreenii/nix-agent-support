...
// This file holds this package's contract-tagged tests: they exercise the
// real `bd` CLI against this machine's shared per-user dolt server
// (127.0.0.1:25252),
// exactly what the pg-test-runner "contract" label is for (mirrors
// pkg/beads/contract_test.go's split in this same module). They were
// previously interleaved with pure-unit tests directly in sync_test.go, so
// they ran under the commit-time `go test -race -run ^(Test|Example) ./...`
// unit gate and collided with each other and with sibling pg-pr test
// binaries under load (pg2-5ek6b/pg2-12795: "Found existing Dolt database
// ... already initialized" and related git-fixture escapes during a real
// commit-time hook run). Moved here, behind the `contract` build tag, they
// no longer compile into the default unit run at all and instead run only
// via `go test -tags contract -p 1 ./...`, or under `nix flake check`'s
// hermetic checks.pg-pr-go-tests tier, where the shared dolt daemon is not
// reachable and this contention cannot occur.
package sync
...
// budget.
//
// The --prefix is unique PER PROCESS (synctest<pid>), not the literal
// "synctest": despite this workspace being an isolated t.TempDir()-rooted
// directory, `bd` on this machine has no true per-directory-embedded mode —
// with BEADS_DOLT_AUTO_START=0 set machine-wide, `bd init` here still talks
// to the ONE shared per-user dolt server (127.0.0.1:25252), which tracks
// prefixes globally. A literal "synctest" therefore collides deterministically
// with ANY earlier run (this session's own, a peer session's, or a stale
// leftover) that already registered that exact name, and also races a
// concurrently-running sibling process using the same fixed name — confirmed
// 2026-08-27 (pg2-5ek6b/pg2-12795): every test that reaches this template hit
// "Found existing Dolt database ... already initialized" under a real
...
func bdWorkspaceTemplate(t *testing.T) string {
	...
	bdTemplateOnce.Do(func() {
		...
		defer cancel()
		prefix := fmt.Sprintf("synctest%d", os.Getpid())
		init := exec.CommandContext(ctx, "bd", "init", "--prefix", prefix)
		init.Dir = dir
		init.Env = env
...
// ----------------------------------------------------------------------
// Tests moved from ci_truncation_test.go (real-bd contract tests)
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// Test moved from draft_promotion_test.go (real-bd contract test)
// ----------------------------------------------------------------------
