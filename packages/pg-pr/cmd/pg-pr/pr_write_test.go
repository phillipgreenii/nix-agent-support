...
// fakeBridgeBeads is a minimal beadsbridge.BeadClient double used by
// swapCascadeCloser below. Formerly defined in sync_test.go (deleted along
// with `pg-pr sync`'s retiring command surface); moved here since this
// package's remaining consumer of the type is `pr close`'s cascade-close
// tests.
type fakeBridgeBeads struct {
	...
	// Handle. nil (the zero value) preserves every other test's existing
	// behavior.
...
// (above) rather than a new type: it already satisfies beadsbridge.BeadClient
// and records every cascade close call.
//
...
// temp dir via setListStateHome). Retained for review_test.go's WIP-gated
// review tests even though pr_write_test.go's own `pr wip` command tests
// were removed with `pr wip on|off`.
