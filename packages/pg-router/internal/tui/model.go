// Package tui implements pg-router's operator-facing terminal UI: the
// Model/Update/View skeleton (the tea.Model interface) and Run, the
// exported entry point cmd/pg-router/tui_cmd.go hands off to (Task 4.5). This
...
type Options struct {
	...
	// PG_ROUTER_TUI_INTERVAL, resolved by cmd/pg-router before calling Run).
	...
	Out io.Writer
	// Version is this TUI binary's own build identifier (cmd/pg-router's
...
// Model implements tea.Model: pg-router's own health grammar rendered over
...
// Run is internal/tui's exported entry point: cmd/pg-router/tui_cmd.go's
...
func (m *Model) View() string {
	...
	case screenQuiescing:
		return "pg-router: quiescing (core.state != \"started\")"
	case screenModal:
