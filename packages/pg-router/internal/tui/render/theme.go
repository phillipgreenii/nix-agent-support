...
// dependency and no dependency on any other pg-router package — it reproduces
...
// Theme holds the display styles internal/tui uses to render pg-router's own
...
// renamed for pg-router's listener/source health axes rather than pa-monitor's
...
func Detect(w io.Writer) (Theme, error) {
	...
	if profile == colorprofile.NoTTY {
		return Theme{}, fmt.Errorf(
			"render: terminal profile %s — the TUI needs a real terminal; run \"pg-router tui\" directly in an interactive shell, not piped, redirected, or under cron",
			profile,
		)
	}
