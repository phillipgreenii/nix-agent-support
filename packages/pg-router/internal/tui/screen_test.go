...
func TestView_ScreenMainRoutesToRenderMain(t *testing.T) {
	...
	got := m.View()
	if got == "" || got == "loading…" || got == "pg-router: main" {
...
func TestNoCoreMessage_SanitizesBeforeComposing(t *testing.T) {
	...
	dirtyPath := "/tmp/\x1b[1mpg-router\x1b[0m/discovery.json"
	...
	if !strings.Contains(got, "/tmp/pg-router/discovery.json") {
...
func TestNoCoreMessage_ShapeMatchesDaemonOfflinePrecedent(t *testing.T) {
	...
	got := noCoreMessage("/var/pg-router/discovery.json", err, render.NewTheme(false), 80, 0)

	for _, want := range []string{
		"No core running",
		"/var/pg-router/discovery.json",
		"pg-router run",
		"daemon",
		"automatically",
		"Press q to quit",
...
func TestNoCoreMessage_FillsRequestedHeight(t *testing.T) {
	...
	t.Run("pads short content up to a taller terminal", func(t *testing.T) {
		got := noCoreMessage("/var/pg-router/discovery.json", err, render.NewTheme(false), 80, 40)
	...
	t.Run("never truncates when content already exceeds the requested height", func(t *testing.T) {
		got := noCoreMessage("/var/pg-router/discovery.json", err, render.NewTheme(false), 80, 3)
		natural := noCoreMessage("/var/pg-router/discovery.json", err, render.NewTheme(false), 80, 0)
		if got != natural {
