...
func TestRegistry_RegisterStartsUnroutable(t *testing.T) {
	...
	reg, err := r.Register("src-beads", KindSource, "pg-router ingest-event --socket s --token t", "pg-router self-status --socket s --token t")
	if err != nil {
...
func TestService_RegisterHandsOutTheCallback(t *testing.T) {
	...
	wantSelfStatus := `pg-router self-status --socket '/s/core.sock' --token 'tok'`
	src, err := svc.Register("src-1", KindSource)
	if err != nil {
		t.Fatalf("Register source: %v", err)
	}
	want := `pg-router ingest-event --socket '/s/core.sock' --token 'tok'`
