...
func startService(t *testing.T, logDir string) (*Service, Ref) {
	t.Helper()
	svc, err := Listen(Options{LogDir: logDir, Queue: newQueue(t), Bindings: testBindings(), Command: "pg-router"})
	if err != nil {
...
func TestListen_RejectsAnOverlongSocketPath(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "PG_ROUTER_LOG_DIR") {
		t.Fatalf("err = %v, want it to name the remedy (PG_ROUTER_LOG_DIR)", err)
	}
}
...
func TestCallbackCommand_CarriesSocketAndTokenQuoted(t *testing.T) {
	svc := &Service{command: "pg-router", ref: Ref{Socket: "/a dir/core.sock", Token: "tok'en"}}
	got := svc.CallbackCommand(SubcommandIngestEvent)
	want := `pg-router ingest-event --socket '/a dir/core.sock' --token 'tok'\''en'`
	if got != want {
...
// (Task 4.1, Binding Decision 2: promoted from cmd/pg-router's private
// discriminateReply, previously only exercised indirectly through
// cmd/pg-router's own integration tests — which no longer counts toward THIS
