...
// Discovery artifact names under Config.LogDir. LogDir is where pg-router already
// keeps its own runtime state (the JSONL event log, PG_ROUTER_LOG_DIR), so the
...
// (Binding Decision 2, Task 4.1: promoted from cmd/pg-router's private
...
// it (cmd/pg-router's status/ingest-event/self-status paths); its new caller
