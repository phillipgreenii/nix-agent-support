// Package eventqueue implements pg-router's durable, ordered, de-duped,
...
// decided in `packages/pg-router/docs/decisions · DEC-EVENT-1`, which amends
...
// coverage is gated at >=90% by the `pg-router-go-tests` flake check (bead
// pg2-hvlyj.19).
package eventqueue
