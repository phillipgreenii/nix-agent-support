...
func TestProduceTick_thenDispatch_matchesBuiltinRoles(t *testing.T) {
	...
	feedbackExt := "pg-router-feedback-zr-c-" + dtest.TestStamp
	workerExt := "pg-router-worker-zr-w1-" + dtest.TestStamp
	...
	if dtest.Contains(cc.Sent, "pg-router-worker-zr-w2-"+dtest.TestStamp) {
...
func TestNewListener_perHandlerSerialFIFO_onePerDispatchCall(t *testing.T) {
	...
	ext2 := "pg-router-worker-zr-w2-" + dtest.TestStamp
	cc := &dtest.FakeCC{ListSeq: [][]ccpool.Session{{
...
func TestWorkOne_workerSuccessWithWatchdogArmed(t *testing.T) {
	...
	cc := &dtest.FakeCC{ListSeq: [][]ccpool.Session{{{ExternalID: "pg-router-worker-zr-w", Live: true, State: ccpool.StateWorking, TranscriptPath: "/t"}}}}
	o := newOrch(cc, bd, cfg)
...
// even when discovery fails" is now a cmd/pg-router run/run-until-idle
...
func TestWorkOne_workerBudgetHardStopUnclaimsNoHuman(t *testing.T) {
	...
	ext := "pg-router-worker-zr-w-" + dtest.TestStamp
...
func TestRunOne_feedbackClosesSession(t *testing.T) {
	ext := "pg-router-feedback-zr-c-" + dtest.TestStamp
...
func TestWorkOne_usesPerAttemptExternalID(t *testing.T) {
	...
	ext := "pg-router-worker-zr-w-" + dtest.TestStamp
	bd := &dtest.ScriptBD{StatusSeq: map[string][]string{"zr-w": {"in_progress", "closed"}}}
	cc := &dtest.FakeCC{ListSeq: [][]ccpool.Session{{{ExternalID: ext, Name: "pg-router-worker-zr-w", Live: true, State: ccpool.StateWorking}}}}
	o := newOrch(cc, bd, cfg)
	d := discover.DispatchContext{Role: workerRole(o), Item: item.Item{ID: "zr-w"}}
	if _, err := o.workOne(context.Background(), d); err != nil {
	...
	if !dtest.Contains(cc.EnsureNames, "pg-router-worker-zr-w") {
		t.Errorf("Ensure must pass the stable DisplayName as --name; names=%v", cc.EnsureNames)
	}
	// Ensure must carry the pgrouter.* dispatch metadata (bead/role/pool) so ccpool
	// stamps it atomically via `new --meta` (pg2-5o5i).
	if cc.EnsuredMeta["pgrouter.bead"] != "zr-w" || cc.EnsuredMeta["pgrouter.role"] != "worker" || cc.EnsuredMeta["pgrouter.pool"] != "pg-router" {
		t.Errorf("Ensure must pass pgrouter.* dispatch metadata; got %v", cc.EnsuredMeta)
	}
}

// TestTeardownAll_purges: teardownAll closes pg-router-prefixed sessions with purge=true.
func TestTeardownAll_purges(t *testing.T) {
	cfg := fastCfg()
	bd := &dtest.ScriptBD{}
	cc := &dtest.FakeCC{ListSeq: [][]ccpool.Session{{
		{ExternalID: "pg-router-worker-zr-x-" + dtest.TestStamp, Live: true},
		{ExternalID: "cc-unrelated", Live: true},
	}}}
	o := newOrch(cc, bd, cfg)
	o.teardownAll(context.Background())
	if len(cc.Closed) != 1 || cc.Closed[0] != "pg-router-worker-zr-x-"+dtest.TestStamp {
...
func TestStuckBead_successClearsLaunchFailLabel(t *testing.T) {
	cfg := fastCfg()
	bd := &dtest.ScriptBD{StatusSeq: map[string][]string{"zr-w": {"in_progress", "closed"}}}
	cc := &dtest.FakeCC{ListSeq: [][]ccpool.Session{{{ExternalID: "pg-router-worker-zr-w-" + dtest.TestStamp, Live: true, State: ccpool.StateWorking}}}}
	o := newOrch(cc, bd, cfg)
	d := discover.DispatchContext{Role: workerRole(o), Item: item.Item{ID: "zr-w"}}
	if _, err := o.workOne(context.Background(), d); err != nil {
		t.Fatalf("worker that closes its bead should succeed, got %v; updates=%v", err, bd.Updates)
...
func TestNewListener_mixedOutcomesAcrossPasses(t *testing.T) {
	...
	cc := &dtest.FakeCC{ListSeq: [][]ccpool.Session{{
		{ExternalID: "pg-router-feedback-zr-ok-" + dtest.TestStamp, Live: true, State: ccpool.StateWorking},
		{ExternalID: "pg-router-feedback-zr-bad-" + dtest.TestStamp, Live: true, State: ccpool.StateWorking},
	}}}
	o := newOrch(cc, bd, cfg)
	ctx := context.Background()
	q := newTestQueue(t)
...
// progress marker (closed=N) — only pg-router-prefixed sessions are counted.
func TestTeardownAll_returnsClosedCount(t *testing.T) {
	...
	if n != 2 {
		t.Errorf("teardownAll closed count = %d, want 2 (pg-router- sessions only); closed=%v", n, cc.Closed)
	}
}

// TestTeardownAll_preservesNeedsInput: a pg-router session in needs_input is left
// alive (NOT closed) so the operator can still attach after the pass; other
// pg-router sessions are still reaped, and the returned count excludes the
...
func TestTeardownAll_preservesNeedsInput(t *testing.T) {
	...
	if dtest.Contains(cc.Closed, "pg-router-worker-zr-need") {
