...
// the pg-router executor's worktree. It is self-contained (no SkillMD). The PR
// coords are templated directly from the review-pr bead's metadata (stamped by
// the pg-router ACL: repo/pr_number/branch/head_sha/ownership). Because the
...
// bead, which pg-router's OWN "feedback" role query already discovers by title
...
func BuiltinRoleSet(p BuiltinParams) RoleSet {
	return RoleSet{
		...
		{
			// review dispatches the review-pr beads the pg-router ACL projects from
