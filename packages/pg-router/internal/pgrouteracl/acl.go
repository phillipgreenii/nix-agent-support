// Package pgrouteracl is the pg-router anti-corruption layer over pg-pr. It reads
...
// for SELECTION (reviewed even while a GitHub draft). It is the pg-router-side copy
...
//
// The predicate is DUPLICATED, not shared, and cannot be shared today: pg-router is
// a separate Go module (github.com/phillipgreenii/pg-router) and pg-pr's ownership
...
// shared: pgrouteracl is an anti-corruption layer over the `pg-pr pr list --json`
...
func ensureReview(ctx context.Context, r beads.Runner, pr PR, mrs, reviews []beads.Issue) (string, error) {
	...
	// switch has no pg-router counterpart; it is not part of this
