...
# built-in roles/queries entirely. With NO config file, pg-router runs exactly these
# built-in feedback + worker + review defaults. Copy this file and edit it to
# customize, or run 'pg-router config --print-defaults' to regenerate it.
#
...
#                  are fixed enums. When authorship_guard = true, pg-router prepends a
...
// default role + query sets. It powers 'pg-router config --print-defaults' and is
...
// tools (bd is pg-router's own first-class dependency; sh/jq are bundled onto
// the pg-router wrapper's PATH — see default.nix — precisely so this generated
