...
pg-router's core is a **dispatcher**: everything outside it is a pluggable **participant** reached
...
```mermaid
flowchart LR
    subgraph core["pg-router core — dispatcher + registry"]
...
- **A message schema, carried over a transport contract.** Every request and reply is defined by the
  ...
  (`phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-WIRE-1`).
...
- **Coarse outcome, rich reply.** A call's outcome is signalled **coarsely by the transport** — it
  ...
  **participant's pre-accept decline** on offered work (`INV-CONC-1`), and the **core's own
  ...
  (`phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-WIRE-1`). The **low** codes
  ...
  (`phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-WIRE-1`). Code `3` is
  ...
  **before any work begins** — no event has yet been offered, no dispatch attempted, nothing has
  ...
  participant's own (`phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-WIRE-1`).
- **Self-status.** Any participant MAY push its **own** status — `healthy` / `degraded` /
  ...
  **pre-accept decline** the core acts on by re-offering the event while it is unexpired
  ...
  one dispatched item, and pg-router takes no per-item progress stream at all. **The push reaches the
...
`phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-EVENT-2`). An event is
...
handler, ccpool's own CLI — which pg-router's implementation already reads directly rather than being
...
**self-status** channel above — is a **different channel**, and pg-router **does** act on it: an
...
and it is deliberately **not** the list of things pg-router measures. pg-router **classifies and counts**
...
- **Emission.** Observability covers **metrics and logs** (traces are a later concern). Both the
  **emission transport** and the concrete backend behind it (a scrape target, a log store) are
  ...
  (`phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-OBS-1`).
...
- **What the operator can do.** The boundary offers exactly these affordances: run the core in its
  ...
  **inspect** a running core for its resolved configuration, its live **deliveries** and its per-`type`
  ...
  (`phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-CLI-2`). _"role"_ is the
...
- **`tui` is not a sixth affordance:** it is the **continuous-interactive** realization of exactly
  ...
  (concrete spelling: `phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions ·
DEC-CLI-2`).
...
per-`type` depth `INV-OBS-1` obliges, and the **resolved configuration** is pg-router's own
...
(`OQ-CONFIG`), with pg-router's existing TOML as prior art.
