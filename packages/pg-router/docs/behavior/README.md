...
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| `GOAL-MIN-1` | the Floor: pg-router speaks in events, bindings, participants, handler sessions, and wiring; it names no concrete tool, transport, tuning constant, or file layout (`## Scope`'s Floor, `GOAL-MIN-1`)                                                                                          | the binary's own contract surface names `bd`/`pg-pr`/`ccpool`/`claude` directly: `--help`'s env-var reference documents `claude` model/effort/permission-mode/allowed-tools and `ccpool`'s default (`cmd/pg-router/args.go:78-81`); the built-in `AllowedTools`, `ReminderMsg` and `WrapUpMsg` defaults hardcode `Bash(bd:*)`, `Bash(pg-pr:*)` and `bd comment {{.BeadID}}` (`internal/config/config.go:225,234-235`); `CCPoolCommand = "ccpool"` and `beads.Command = "bd"` are compiled-in tool names (`internal/config/config.go:157`, `internal/beads/runner.go:22`); and the ACL/drain paths exec `pg-pr` directly (`internal/pgrouteracl/acl.go:315`, `cmd/pg-router/drain.go:95`) — distinct from the pre-existing `GOAL-MIN-1` row (payload-path opacity) and from R15 (participant implementations in the generic binary): this is the Floor's tool-naming clause specifically | bead `pg2-vl05m`    |
