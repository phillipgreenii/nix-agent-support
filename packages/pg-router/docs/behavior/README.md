...
- **Floor** — pg-router speaks in events, bindings, participants, handler sessions, and wiring. It
  ...
  **external** tools, most pointedly an external PR-management tool, which pg-router takes only as
  opaque configuration, never a literal. **One permanent exception** (operator ruling, 2026-09-14):
  `claude`, `bd`, and `ccpool` — `ccpool` is pg-router's own core executor — MAY be named directly
  in the contract surface. Naming that fixed stack is not naming a swappable external tool, so the
  exception narrows this clause rather than contradicting it, and it is settled: this naming MUST
  NOT be stripped as if it were an oversight.
