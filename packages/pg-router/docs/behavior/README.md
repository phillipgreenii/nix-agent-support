...
but pg-router invokes it and never reads it, so it is a **pass-through** and belongs.
...
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| `GOAL-MIN-1` | the Floor's tool-naming clause, NARROWED by operator ruling (2026-09-14, recorded on `pg2-oju6w.13`): pg-router's own contract surface MUST name no EXTERNAL PR-management tool; `claude`/`bd`/`ccpool` naming (`ccpool` is pg-router's own core executor) is a permanent, accepted Floor exception and MUST NOT be stripped — distinct from the pre-existing `GOAL-MIN-1` row (payload-path opacity) and from the (now-deleted) R15 row (participant implementations in the generic binary) (`## Scope`'s Floor, `GOAL-MIN-1`) | the `pg-pr` literal is stripped from the binary's own contract surface: `--help`'s env-var reference, the built-in `AllowedTools` default, and the ACL/preflight exec paths now take the external tool name as configuration (`PRTool`/`PG_ROUTER_PR_TOOL`) instead of hardcoding `pg-pr` (`internal/config/config.go`, `internal/pgrouteracl/acl.go`, `cmd/pg-router-ccpool-handler/preflight.go` — bead `pg2-pjj6j`, commit `7c2a6b86`); `claude`/`bd`/`ccpool` naming remains compiled in by design (`internal/config/config.go`, `internal/beads/runner.go`) — accepted per the ruling above, not a gap                                                                                                             | bead `pg2-vl05m` (closed) |
...
interfaces, whose elements pg-router's own implementation cites). The **what it is** column is one
...
| `INV-CCPOOL-6`   | a handler run held for a human decision is preserved, not reaped, and the accepting handler owns its resume             | `your-private-flake · modules/zm/pg-router/docs/behavior`        | [a5f2e14b-1a49-4bfd-be44-69acc603d685](https://github.com/your-org/your-private-flake/blob/main/modules/zm/pg-router/docs/behavior/invariants.md) |
