# Journeys & open questions — pg-router
...
travels the arc from a source to an accepting handler, where pg-router's interest ends.
...
**data** is durable
(`INV-EVT-1`, `phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-EVENT-2`):
...
trigger. The core carries **no branch or deadletter path** of its own: **delivery is pg-router's
...
(`phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-WIRE-1`). Once the event
...
**So the only failures pg-router itself classifies are two, and both are pre-acceptance or the core's
...
(`phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-OBS-1`). A sink may be
...
`phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-CLI-2`). What the operator
...
- **Serialize marks** — any event type **marked to serialize** (`INV-CONC-1`) is declared **in this
  ...
  (`phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-CONC-1`). So marked,
...
- **Re-entry** — where a handler produces **new work**, it is routed back in as events **via a
  query**. The core does **not** branch outputs itself: **delivery is pg-router's contract and
...
`phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-CLI-1`). The restriction
...
- **`OQ-CONFIG`** <!-- uuid: e004fda6-f6e6-41e1-8ec4-0b90c17fd2b2 --> — the full **configuration schema**: participants (command + mode), event sources,
  ...
  operator/author. _Path_: extract from pg-router's TOML prior art and pin the schema. _Blocks_:
