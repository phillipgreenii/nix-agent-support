# Concurrency — pg-router decision docs

Realization decisions about pg-router's **serialize-mark configuration surface** — the concrete way an
...
lets the next event of a marked type be offered — is in pg-router's
...
(`cmd/pg-router/run.go`'s `bootCore`) — the same overlay/threading shape `[pool].retry` and
