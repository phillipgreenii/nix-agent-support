# Wire — pg-router decision docs

Realization decisions about **how** a message crosses one of pg-router's participant boundaries: the
...
`INV-INTF-1`, `INV-INTF-2` and the interface definitions in pg-router's
...
```json
{
  ...
  "accepted": true,
  "callback": "",
  "selfStatusCallback": "pg-router self-status --socket /run/pg-router.sock --token tok-abc"
}
```
...
```json
{
  "schemaVersion": "1",
  "accepted": true,
  "callback": "pg-router ingest-event --socket /run/pg-router.sock --token tok-abc",
  "selfStatusCallback": "pg-router self-status --socket /run/pg-router.sock --token tok-abc"
}
```
...
self-restart-on-version-skew decision) is the precedent for that mismatch case, and pg-router
**deviates from it**: rather than re-executing itself on a detected skew, as pa-monitor's separate
daemon/client binaries do, pg-router's client shows a hint and relies on the operator to restart. That
...
daemon and any such client in pg-router (unlike pa-monitor's separate binaries), so a version mismatch
...
sequenceDiagram
    ...
    participant CLI as pg-router ingest-event
    participant Core as core (socket service)
    Src->>CLI: pg-router ingest-event  (event JSON on stdin, socket+token from callback)
