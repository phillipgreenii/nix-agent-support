...
import (
	...
	"github.com/phillipgreenii/pg-router/internal/pgrouteracl"
	"github.com/phillipgreenii/pg-router/internal/reconcile"
)

// runReconcile implements `pg-router reconcile`. It does two things:
//
...
// and is exit-0-on-partial so a following `pg-router drain` is never stranded.
func runReconcile() int {
	...
	return reconcileACL(ctx, os.Stdout, br, cfg.RepoRoot, pgrouteracl.ReadPRList)
}
...
// aborted. pg-router's own preconditions (config/self) remain hard failures in the
// caller.
//
...
// pg-router is the intended SOLE review owner, so gating this ACL on it would
// leave ZERO review owners at the shipped default. pg-router therefore does not
// read pg-pr's config at all — the only seam is the `pg-pr pr list --json` CLI
// (pgrouteracl.ReadPRList), which carries PR facts only, and it MUST NOT be
// extended to carry pg-pr configuration state. An operator stops this producer
// by not invoking `pg-router reconcile` (there is no flag or config key for it).
func reconcileACL(
	ctx context.Context,
	w io.Writer,
	br beads.Runner,
	repoRoot string,
	listFn func(context.Context, string) ([]pgrouteracl.PR, error),
) int {
	...
	ids, errs := pgrouteracl.Reconcile(ctx, br, prs)
	for _, e := range errs {
