...
// This file is pg-router's Cluster-5 end-to-end test (bead pg2-f3mcb.5): it builds
// the REAL ./cmd/pg-router binary and execs it TWICE as genuinely separate OS
...
// the nix build sandbox where bd is only wired onto pg-router's own runtime PATH
...
// passes with no PG_ROUTER_BEADS_PREFIX override needed.
...
// buildPgRouterBinary compiles the REAL cmd/pg-router main package this test file
...
// package-level fake.
func buildPgRouterBinary(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "pg-router")
	ctx, cancel := context.WithTimeout(context.Background(), 120*time.Second)
	defer cancel()
	cmd := exec.CommandContext(ctx, "go", "build", "-o", bin, ".")
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("go build ./cmd/pg-router: %v\n%s", err, out)
	}
	return bin
}

// writeE2EConfig writes a real <repoRoot>/.pg-router/config.toml declaring ONE
...
func writeE2EConfig(t *testing.T, repoRoot, markerPath string) {
	t.Helper()
	dir := filepath.Join(repoRoot, ".pg-router")
...
func TestE2E_RunUntilIdlePushInject_RealBinary(t *testing.T) {
	...
	t.Setenv("PG_ROUTER_REPO_ROOT", repoRoot)
