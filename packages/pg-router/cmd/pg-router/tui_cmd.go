...
// envTUIInterval is PG_ROUTER_TUI_INTERVAL, spelled out as its own constant
// (mirroring envSocket/envToken in ingest_event.go) since runTUI is its only
// reader.
const envTUIInterval = "PG_ROUTER_TUI_INTERVAL"

// tuiIntervalDefault is the TUI poller's built-in refresh interval absent any
// PG_ROUTER_TUI_INTERVAL override (spec §11, comp-8's env-vs-default half of
...
// runTUI implements `pg-router tui [--socket ...] [--token ...]` (Task 4.2):
...
// concern — none exists yet): PG_ROUTER_TUI_INTERVAL, if set and parseable,
