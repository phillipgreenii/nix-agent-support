...
func TestRoute_pushInject(t *testing.T) {
	r := route([]string{"pg-router", "push-inject", "--json", "--socket", "/s", testPushEvent})
...
// The vocabulary defect: pg-router's subcommand is `push-inject`. There is no
// `pg-router-emit`, so nothing operator-facing may advertise one.
func TestHelpText_DoesNotAdvertisePgRouterEmit(t *testing.T) {
	if strings.Contains(helpText, "pg-router-emit") {
		t.Fatal("helpText advertises a `pg-router-emit` subcommand, which does not exist")
	}
}

// Operator ruling (2026-09-02): pg-router has no live consumers, so it carries
...
// PG_ROUTER_TOKEN is consulted only when the socket also came from the environment.
