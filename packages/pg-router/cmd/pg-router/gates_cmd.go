...
// gained a per-connector AsOf/Stale contract (bead pg2-4aoeg), pg-router grew a
...
// runResume implements `pg-router resume [<gate>] | --all` against the real
...
func pauseGate(stdout, stderr io.Writer, gate string) int {
	...
	now := time.Now()
	body := fmt.Sprintf("paused by `pg-router pause %s` at %s\n", gate, now.UTC().Format(time.RFC3339))
