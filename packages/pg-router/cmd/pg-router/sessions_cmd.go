...
// sessionRow is one pg-router session resolved from session metadata.
...
// collectPoolSessions finds this pool's sessions (ListByMeta on pgrouter.pool) and
...
// runSessions implements `pg-router sessions`: list this pool's sessions with bead/role
