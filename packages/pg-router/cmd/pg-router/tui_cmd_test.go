...
func TestRoute_tui(t *testing.T) {
	r := route([]string{"pg-router", "tui", "--socket", "/s", "--token", "t"})
	if r.kind != routeTUI {
...
// args_test.go's TestHelpText_MentionsActivityRingEnvVar). PG_ROUTER_TUI_INTERVAL
// is new operator-facing surface introduced by this packet.
func TestHelpText_MentionsTUIIntervalEnvVar(t *testing.T) {
	if !strings.Contains(helpText, "PG_ROUTER_TUI_INTERVAL") {
		t.Fatal("helpText does not mention PG_ROUTER_TUI_INTERVAL")
	}
}
