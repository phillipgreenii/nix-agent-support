...
// `phillipgreenii-nix-agent-support · packages/pg-router/docs/decisions · DEC-CLI-1`:
// --only (allow-list) / --disable (deny-list), wired onto `run` and
// `run-until-idle` only (parseRunLikeArgs, args.go). This file holds the
// selector grammar, the repeatable flag.Value, the PG_ROUTER_ONLY/PG_ROUTER_DISABLE
...
// envOnly / envDisable are PG_ROUTER_ONLY / PG_ROUTER_DISABLE (DEC-CLI-1): each a
...
const (
	envOnly    = "PG_ROUTER_ONLY"
	envDisable = "PG_ROUTER_DISABLE"
)
...
// PG_ROUTER_ONLY/PG_ROUTER_DISABLE equivalents.
...
// PG_ROUTER_ONLY/PG_ROUTER_DISABLE. Reading the environment deliberately does NOT
...
// the same split push_inject.go uses (injectedRef reads PG_ROUTER_SOCKET/
// PG_ROUTER_TOKEN in runPushInject, never in route()).
//
...
// PG_ROUTER_DISABLE in their environment and adds one more --disable on this
...
// splitEnvList parses a PG_ROUTER_ONLY/PG_ROUTER_DISABLE value: a comma-separated
...
func checkSmokeReachable(kind, name string, sel runSelectors) error {
	...
	return fmt.Errorf("%s %q is excluded by the active selectors (PG_ROUTER_ONLY/PG_ROUTER_DISABLE)", kind, name)
}
...
// cmd/pg-router's bootCore, which threads it into
