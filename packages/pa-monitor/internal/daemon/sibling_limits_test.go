...
// TestSiblingLimitsSource_NewestAcrossFilesByTS is the core reader contract (ADR
// 0021 §1): return the single most-recent record across ALL *.status.jsonl files
// ordered by the embedded ts, ignoring session_id entirely.
func TestSiblingLimitsSource_NewestAcrossFilesByTS(t *testing.T) {
	...
	// Newest ts is 1700000200 (sess-b), even though sess-a has two records.
	if got.FiveHourPct == nil || *got.FiveHourPct != 34 {
		t.Errorf("FiveHourPct = %v, want 34 (newest record by ts)", got.FiveHourPct)
	}
