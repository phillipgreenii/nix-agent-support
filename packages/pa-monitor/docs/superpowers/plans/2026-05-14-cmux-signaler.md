...
- The existing "no signaler for pid X" log line is acceptable (tmux users already see it). If it appears for _every_ session every cycle, that is pre-existing behavior, not a regression introduced by this task.
