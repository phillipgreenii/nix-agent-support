...
```jsonc
{
  ...
  "message": {
    ...
    "content": [
      {
        "type": "text",
        "text": "You've hit your limit · resets 3:20pm (America/New_York)",
      },
    ],
  },
  "error": "rate_limit",
...
The `error` field is polymorphic: an object in the old shape, a string in the new shape. Keep the existing `scanEv` (with object-shaped `Error`) and attempt a _second_ unmarshal of each line into a small auxiliary struct that has `Error string`. The two coexist because Go's `json.Unmarshal` errors when type-mismatched; the second try simply leaves the auxiliary fields zero when the old shape is present.
...
A _real_ (non-synthetic) assistant or user event after the rate-limit still clears the pause via the existing `resumedAfterAPIErr` path.
