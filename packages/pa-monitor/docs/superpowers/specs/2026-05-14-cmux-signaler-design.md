...
One cmux daemon serves one socket; every window / workspace / pane / panel / surface is reachable from any in-cmux process. Per `cmux --help`, `CMUX_WORKSPACE_ID` is used as the _default_ `--workspace` for every command — so naive `cmux list-pane-surfaces` would only enumerate the caller's own workspace and miss agents running in sibling workspaces.
...
The "no signaler for pid X" log line that `signalNonWorking` already emits when _all_ signalers miss is unchanged — it predates this work and is identical for tmux users.
...
- `cmux list-pane-surfaces` returns only the _caller's_ pane (one surface in our capture), not a per-workspace listing — useless for the cross-workspace need.
