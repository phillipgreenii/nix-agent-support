...
func TestMain(m *testing.M) {
	if err := os.Setenv(BinEnvVar, filepath.Join(os.TempDir(), "pg-desk-test-must-never-be-a-real-browser")); err != nil {
...
// TestOpenWindowConfigBinTakesPrecedenceOverEnvVar pins this package's own
// deviation from the pg-pr original (see browser.go's package doc comment):
// an explicit bin argument — the config-driven open.chrome_bin — wins over
// BinEnvVar, matching D10's "always a variable read from config" requirement.
func TestOpenWindowConfigBinTakesPrecedenceOverEnvVar(t *testing.T) {
	envRecord := stubChrome(t, 0)

	cfgDir := t.TempDir()
	cfgRecord := filepath.Join(cfgDir, "argv")
	cfgBin := filepath.Join(cfgDir, "chrome-stub-cfg")
	script := "#!/bin/sh\n" +
		"for a in \"$@\"; do printf '%s\\n' \"$a\" >> \"" + cfgRecord + "\"; done\n" +
		"exit 0\n"
	if err := os.WriteFile(cfgBin, []byte(script), 0o755); err != nil {
		t.Fatalf("write cfg chrome stub: %v", err)
	}

	if err := openWindow([]string{"https://example.test/pull/1"}, cfgBin); err != nil {
		t.Fatalf("openWindow() error = %v", err)
	}
	if got := recordedArgv(t, envRecord); got != nil {
		t.Errorf("the env-var stub must NOT have been invoked when a config bin is given, got: %v", got)
	}
	if got := recordedArgv(t, cfgRecord); len(got) == 0 {
		t.Errorf("the config-driven stub was not invoked")
	}
}
