...
const (
	...
	//
	// Ported verbatim from packages/pg-pr/internal/browser/browser_darwin.go.
	// Overridden, in precedence order, by an explicit bin argument to
	// openWindow (the config-driven open.chrome_bin), then BinEnvVar.
...
func openWindow(urls []string, bin string) error {
	if len(urls) == 0 {
		return nil
	}

	resolvedBin := DefaultBin
	...
	if _, err := os.Stat(resolvedBin); err != nil {
		return fmt.Errorf("cannot find the Google Chrome executable at %s (set open.chrome_bin or %s to override): %w", resolvedBin, BinEnvVar, err)
	}
	...
	if err := cmd.Start(); err != nil {
		return fmt.Errorf("launch %s: %w", resolvedBin, err)
	}
	...
	case err := <-done:
		if err != nil {
			return fmt.Errorf("%s exited with error: %w (output: %s)", resolvedBin, err, strings.TrimSpace(out.String()))
		}
		return nil
