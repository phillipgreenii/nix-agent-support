...
| `INTF-CCH-PGPR` | a handler session's ACL check consulting `pg-pr` (zero or more per handler session, depending on configuration)                                               | `internal/pgrouteracl` carries the full crossing (`Reconcile`/`ReadPRList`, network-free `pg-pr pr list --json` read + review-pr bead projection), moved verbatim from pg-router's old `reconcile` subcommand (`docs/adr/0065`'s "Move list") — but per that ADR's superseded-deprecation-shim ruling the subcommand wiring was deleted outright, not ported, so no dispatch/subcommand path in this module invokes it today; wiring a replacement (a `push-inject` source or role) is left to a downstream deployment, not yet built here | `pg2-7d9mm`   |
