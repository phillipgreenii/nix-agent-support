...
| `INTF-SOURCE`  | this module realizes `INTF-SOURCE` for the beads-backed built-in query set                 | the `query` subcommand exists and speaks the wire contract, but always answers zero events — the beads client has moved here (`internal/beads`), but the built-in query set itself (`packages/pg-router`'s `internal/roles`/`internal/query`) has not; wiring it for real is Task 5.8 | `pg2-oju6w.8` |
