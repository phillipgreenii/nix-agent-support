...
| `INV-CCH-2`     | a handler session MUST support the deferred-ack reply form (the duplicate-tolerance half is realized — see `internal/executor/ccpool.go`'s `absorbDuplicate`) | not yet: `dispatch` replies INLINE for every dispatch today (`cmd/pg-router-ccpool-handler/dispatch.go`'s own doc comment) — the `deferred` branch `handler.dispatch-reply.schema.json` declares is never taken. Same underlying residual `INTF-HANDLER`'s row above already names (a long-running ccpool session correctly holds the call open rather than a daemonized deferred reply), traced here against its own element id too                                                                                                                                                                                                                                                                                 | `pg2-oju6w.4`             |
| `INTF-CCH-PGPR` | a handler session's ACL check consulting `pg-pr` (zero or more per handler session, depending on configuration)                                               | no implementation exists: `internal/pgrouteracl` had carried the crossing (`Reconcile`/`ReadPRList`, network-free `pg-pr pr list --json` read + review-pr bead projection), moved verbatim from pg-router's old `reconcile` subcommand (`docs/adr/0065`'s "Move list") — but per that ADR's superseded-deprecation-shim ruling the subcommand wiring was deleted outright, not ported, so no dispatch/subcommand path in this module ever invoked it; with no caller and no near-term plan to build a replacement (a `push-inject` source or role), the dead package itself was deleted outright (operator decision, bead `pg2-gidpd`) — wiring a replacement is left to a downstream deployment, not yet built here | not yet tracked by a bead |
