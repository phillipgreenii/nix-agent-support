...
func TestReconcileClosedBeadSessions_closesIdleWithClosedBead(t *testing.T) {
	...
	n := reconcileClosedBeadSessions(context.Background(), cc, (&fakeWorktreeOpener{}).Open, br, "pg-router-")
	if n != 1 {
...
func TestReconcileClosedBeadSessions_leavesUntaggedSessionAlone(t *testing.T) {
	...
	n := reconcileClosedBeadSessions(context.Background(), cc, (&fakeWorktreeOpener{}).Open, fakeBR{}, "pg-router-")
	if n != 0 || len(cc.Closed) != 0 {
...
func TestReconcileClosedBeadSessions_removesWorktreeOfClosedSession(t *testing.T) {
	...
	n := reconcileClosedBeadSessions(context.Background(), cc, open.Open, br, "pg-router-")
	if n != 1 {
