...
// Integration/smoke tests that exercise pg-router's bd client against a REAL `bd`
// binary backed by an isolated, throwaway embedded-Dolt store (NOT the shared
// dolt sql-server). They are the guard that keeps pg-router's parsing in sync with
...
// bd is only wired onto pg-router's runtime PATH via wrapProgram) or under
// `go test -short`.
...
// the exact worker discovery query, run through pg-router's own Ready() against a
...
// envelope failed pg-router's bare-array decode.
