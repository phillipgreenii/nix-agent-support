...
// 2026-08-27 (pg2-lx41y for pg-pr; full mechanism write-up on pg2-67h4y)
// under a leaked GIT_DIR:
//
...
// every descendant process inherits them. So any pg-router process launched
// from a git hook (or under `git rebase`, `git bisect run`, or any tool that
// was itself launched from a hook) runs with the leak, and a mutating verb —
// like watchdog's `reset --hard` / `clean -fd` — then acts on the leaked
// repository instead of the worktree the caller asked for. In a linked
// worktree `git config` writes to $GIT_COMMON_DIR/config, i.e. the canonical
// clone's config shared by every worktree, which is exactly how the
// canonical clone's own .git/config was corrupted in a sibling incident
// (pg2-12795 / pg2-5ek6b). A plain-clone commit hook exports GIT_INDEX_FILE
// but NOT GIT_DIR, which is why a standalone `prek run` never reproduces
// this and only worktree commits do.
//
// Every `git` child in this module's production code MUST therefore be built
// by [Command], which owns the child environment. A bare
...
// a future git release) is excluded automatically. Same allowlist-inversion
// design already used in this workspace for the identical defect class:
// pg-pr's internal/gitenv (pg2-lx41y), claude-extended-tool-approver's
// gh.hermeticGitEnviron (pg2-2pokz), and ccpool's gitfacet.hermeticEnviron
// (pg2-aqpvr).
...
// (inline config injection); GIT_AUTHOR_* and GIT_COMMITTER_* (a hook
// exports these for the commit in progress; no verb here commits, and the
// target repo's own config is the right identity source); and
// GIT_EXEC_PATH plus the GIT_TRACE* family (git recomputes the former from
// its own location, and the latter would inject trace output into stderr
// that callers fold into error messages).
var inheritableGitVars = map[string]struct{}{
	...
	// Interaction policy. Dropping GIT_TERMINAL_PROMPT=0 would let a call
	...
	// these to sandbox pg-router, so forwarding them is a safety feature
	// rather than a leak, and none of them names a repository. Contrast
