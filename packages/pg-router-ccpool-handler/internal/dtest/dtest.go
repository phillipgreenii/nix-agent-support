// Package dtest provides shared test fakes for this module's internal packages.
// It is a normal (non-_test.go) file so it can be imported by multiple test packages.
//
// Duplicated from packages/pg-router/internal/dtest (still needed there by
// its own orchestrator/run tests, though those callers' own use of
// dtest.FakeCC/ScriptBD is now among the expected-broken casualties of this
// move — Task 5.4's job, not this one's): its own ccpool/beads/usage
// dependencies moved here wholesale, so a copy re-pointed at this module's
// own copies is the natural mirror, not a fresh design (docket pg2-oju6w
// Task 5.2/5.3, folded per the operator's 2026-09-11 decision; docs/adr/0065's
// Addendum).
package dtest
