...
// legendRows is the hand-curated symbol list shown by LegendModal, one row
// per DefaultGlyphs entry — pr-pool's own health grammar (see glyphs.go).
var legendRows = []ModalRow{
	{Left: DefaultGlyphs.OK, Right: "ok         actively producing output"},
	{Left: DefaultGlyphs.Cooling, Right: "cooling N s  backing off after a failure; resumes in N s"},
	{Left: DefaultGlyphs.Failing, Right: "failing ×N   failed N times in a row (see status)"},
	{Left: DefaultGlyphs.Paused, Right: "paused     the operator paused this listener/source"},
	{Left: DefaultGlyphs.Disabled, Right: "disabled   disabled at the configuration level"},
	{Left: DefaultGlyphs.Excluded, Right: "excluded   excluded from this run by a selector"},
	{Left: DefaultGlyphs.Stale, Right: "stale      hasn't ticked recently (see last tick)"},
}
