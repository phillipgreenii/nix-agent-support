...
func TestReopenReview_SetsStatusOpenAndRefreshesMetadata(t *testing.T) {
	f := &fakeRunner{}
	if err := ReopenReview(context.Background(), f, "zr-rv7", "newsha", "feat/x"); err != nil {
	...
	want := "update zr-rv7 --status=open --set-metadata head_sha=newsha --set-metadata branch=feat/x --assignee="
	if got != want {
...
func TestReopenReview_ErrorPropagates(t *testing.T) {
	...
	err := ReopenReview(context.Background(), f, "zr-rv7", "newsha", "feat/x")
	if err == nil {
