...
| `INTF-SOURCE`           | a pull `query` and a push `ingest-event` call are messages over `INTF-SOURCE`'s query-reply contract, under the common manager contract's rule that every core→participant request carries a per-call **tracking id** so a deferred reply's later callback correlates back to the originating call (interfaces.md's common contract, "Tracking id") | the core calls `query.Query.Run` as an in-process Go method call (`internal/discover/discover.go:272`) whose signature carries no tracking id and no callback (`internal/query/query.go:68`), so no `query`/`ingest-event` message ever crosses the core<->participant boundary and no tracking id is ever minted — distinct from the pre-existing `USECASE-CREATE-SOURCE` row (`pg2-u7rzl`), which reads the same code fact from the use case's reply-shape side: this row is `INTF-SOURCE`'s own message-boundary/tracking-id clause                                                                                                                                                                                                                                                                                                                                            | bead `pg2-nr1xm`               |
| `INTF-HANDLER`          | dispatch to a handler is a message over `INTF-HANDLER`'s dispatch contract carrying the common manager contract's per-call **tracking id** (`INV-INTF-1`); the deferred form of that reply IS the handler's acceptance, so no later callback need follow it (interfaces.md's common contract, "Deferred replies")                                   | the core calls `roleListener.Offer(evt eventqueue.Event) bool` as an in-process Go method call (`internal/orchestrator/listener.go:80-86`) whose signature carries no tracking id and returns a bare bool — by the type's own doc comment "there is no deferred/async form on this bridge" (`listener.go:76`) — so no dispatch message ever crosses the core<->handler boundary and no tracking id is ever minted for that seam — distinct from the pre-existing `USECASE-CREATE-SOURCE` row (`pg2-u7rzl`), which reads the same code fact from the use case's reply-shape side across both interfaces, and the `INTF-SOURCE` row (`pg2-nr1xm`), the source-side message boundary: this row is `INTF-HANDLER`'s own message-boundary/tracking-id clause specifically                                                                                                              | bead `pg2-q6tqg`               |
