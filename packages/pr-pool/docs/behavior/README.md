...
| `INV-DISP-3`     | an event whose `type` no configured binding declares is **rejected** to the caller and never enqueued | ingest still **accepts** an unbound `type` (exit `0`) and enqueues it to expire unconsumed: `packages/pr-pool/internal/core/ingest.go:70` states that rule and `internal/core/ingest_test.go:95` asserts it                                                                            | bead `pg2-nh25m`   |
| `INV-WORKFLOW-1` | six pre-runtime wiring checks, each one blocking startup                                              | `packages/pr-pool/internal/config/config.go:234` `Validate` implements two of the six — orphan consumer (`:258`) and orphan producer (`:268`) — and no check exists for a disconnected handler, a handler with no events to listen for, an absent backing command, or a re-entry cycle | bead `pg2-0xa2n`   |
