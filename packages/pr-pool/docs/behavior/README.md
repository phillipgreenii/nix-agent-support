...
| ------------ | ------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| `INV-CONC-1` | a `type` MAY be marked to **serialize**, so events of that type never run in parallel | no per-type serialization exists in the dispatch path — the queue's per-listener serial FIFO is not a per-type mark, and nothing reads such a mark | bead `pg2-cl9jz` |
