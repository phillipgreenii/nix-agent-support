...
| `INV-LIFE-1` | the core boots via `run` (daemon) or `run-until-idle` (drain-and-exit) and dispatches from the durable queue, staying reachable to push participants throughout (`internal/core.Service.Listen` + `Accept`) | `cmd/pr-pool` has no `run`/`run-until-idle` subcommand, and nothing outside `internal/core`'s own tests calls `Listen`+`Accept`; production `drain` instead runs the pre-convergence `internal/orchestrator` + `internal/eventbus` path, which `pg2-f3mcb.2` subsumes | bead `pg2-f3mcb.2` |

## External references

This set follows the behavior-docs method and cites elements the method defines, and its
implementation cites elements a downstream deployment set defines — and, separately, elements
**pg-pr** defines: pr-pool's `reconcile` operator command is a driving consumer of pg-pr's
machine read seam (`INTF-PGPR-READ`), and its find-or-reuse logic depends on pg-pr remaining the
sole creator of merge-request records (`INV-MR-1`); this is a direct pg-pr↔pr-pool seam, not
mediated by the ZR deployment. Each external element it references is declared here with the
owner's UUID, so a cross-set reference resolves by the owner's UUID — not the mutable name (a
rename never breaks the seam). The owner set-path is the cited `<repo> · <set-path>`, and this
table spans **three** owner sets: the **method** set (the rules this set follows), the **ZR
deployment** set (the deployment that implements these interfaces, whose elements pr-pool's own
implementation cites), and **pg-pr** (the PR-data owner pr-pool's `reconcile` command reads from
directly). The **what it is** column is one line, so a reader learns why the row is there without
following the reference, and the UUID is rendered as a link to the owner's remote-served
definition. **The UUID is the authority; the URL may rot** — a dead link is an inconvenience,
never a broken identity. A row declares **one** element: where the implementation cites a whole
family (`INTF-ZR-*`), the family resolves through the declared member below.
...
| `INTF-PGPR-READ` | PR facts out, with freshness — the machine read seam pr-pool's `reconcile` command drives as consumer                   | `phillipgreenii-nix-agent-support · packages/pg-pr/docs/behavior` | [8906d9c1-cc11-4f4d-90b3-0a589061d2ce](https://github.com/phillipgreenii/nix-agent-support/blob/main/packages/pg-pr/docs/behavior/interfaces.md)      |
| `INV-MR-1`       | pg-pr is the sole creator of merge-request tracking records — pr-pool finds-or-reuses one, never creates one itself     | `phillipgreenii-nix-agent-support · packages/pg-pr/docs/behavior` | [bacb5392-230f-4cf7-a10b-6821ddb0f085](https://github.com/phillipgreenii/nix-agent-support/blob/main/packages/pg-pr/docs/behavior/invariants.md)      |
| `INV-ASOF-1`     | a consumer MUST NOT act on a read pg-pr flags stale — the freshness gate pr-pool's `reconcile` command honors           | `phillipgreenii-nix-agent-support · packages/pg-pr/docs/behavior` | [39888347-cfb9-45fd-b72e-b465e2337d15](https://github.com/phillipgreenii/nix-agent-support/blob/main/packages/pg-pr/docs/behavior/invariants.md)      |
