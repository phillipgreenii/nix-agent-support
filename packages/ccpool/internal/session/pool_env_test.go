...
func TestLaunchAndWait_injectsPool(t *testing.T) {
	...
	if _, err := s.launchAndWait(ctx, "a", "cc-a", "csid", "name-a", "/cwd", 0, []string{"claude"}, nil, false, "brand_new"); err != nil {
...
func TestLaunchAndWait_defaultModeNoPool(t *testing.T) {
	...
	_, _ = s.launchAndWait(ctx, "a", "cc-a", "csid", "name-a", "/cwd", 0, []string{"claude"}, nil, false, "brand_new")
