...
//
// Fanout lets cmd/ccpool keep its existing stderr text logging while
// additionally exporting every record over OTLP through the otelslog
// bridge. NewSlogHandler builds that bridge bound to the global
// LoggerProvider Init installs. Mirrors packages/pg-router/internal/
// telemetry/slog.go and packages/pg-desk/internal/telemetry/slog.go
// exactly (pg2-qye99 / pg2-24f89).
package telemetry
...
// scopeName is the OTel instrumentation scope name passed to
// otelslog.NewHandler. It does not set service_name — that comes from the
// resource (hardcoded to "ccpool") buildResource configures in Init.
const scopeName = "github.com/phillipgreenii/ccpool"
...
// name is scopeName; it does NOT set service_name.
